...
import { FolderOpen, ImagePlus, Library, Minus, Pencil, Square, Upload } from 'lucide-react';
...
export type LibraryTileset = { definition: TilesetDefinition; blob: Blob; url: string };
...
function targetFromPointer(event: PointerEvent<HTMLButtonElement> | MouseEvent<HTMLButtonElement>, tool: ObstacleTool, tilesetKind: TilesetDefinition['kind']): HoverTarget {
  ...
  return {
    ...
    edge: tool === 'edge' && tilesetKind === 'grid' ? nearestCollisionEdge(x, y) : undefined,
    boundary: tool === 'edge' && tilesetKind === 'a2' ? true : undefined,
  };
}
...
function TilesetCanvas({ tileset, imageUrl, tool, onChange }: {
  ...
  const regionWidth = tileset.kind === 'a2' ? tileset.tileSize * 2 : tileset.tileSize;
  const regionHeight = tileset.kind === 'a2' ? tileset.tileSize * 3 : tileset.tileSize;
  ...
  >
    ...
    {tileset.terrains.map(terrain => {
      ...
      const edges = terrain.collision.kind === 'edges' && terrain.collision.edges.length
        ? tileset.kind === 'a2' ? COLLISION_DIRECTIONS : terrain.collision.edges
        : [];
      return <button
        key={terrain.id}
        type="button"
...
function LibraryDialog({ open, onOpenChange, game, library, onImport }: {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  game: SourceGame;
  library: LibraryTileset[];
  onImport: (asset: LibraryTileset) => void;
}) {
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[82vh] max-w-3xl grid-rows-[auto_1fr_auto] overflow-hidden p-0 sm:max-w-3xl">
    <DialogHeader className="border-b px-5 py-4"><DialogTitle>Tileset Library</DialogTitle><DialogDescription>Choose a bundled tileset to copy into this project.</DialogDescription></DialogHeader>
    <div className="grid min-h-0 gap-3 overflow-y-auto p-5 sm:grid-cols-2">{library.map(asset => {
      const imported = Boolean(game.tilesets[asset.definition.id]);
      return <article key={asset.definition.id} className="flex gap-3 border bg-muted/10 p-3">
        ...
        <div className="min-w-0 flex-1"><div className="truncate font-medium">{asset.definition.name}</div><div className="mt-1 text-xs text-muted-foreground">{asset.definition.kind.toUpperCase()} · {asset.definition.category}</div><Button className="mt-4" size="sm" disabled={imported} onClick={() => onImport(asset)}>{imported ? 'Imported' : 'Import'}</Button></div>
      </article>;
    })}</div>
...
function ImportDialog({ open, onOpenChange, onImport }: {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-xl sm:max-w-xl">
    <DialogHeader><DialogTitle>Import a local PNG</DialogTitle><DialogDescription>Add a PNG as a grid or RPG Maker A2 autotile tileset.</DialogDescription></DialogHeader>
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2"><Label htmlFor="tileset-format">Format</Label><select id="tileset-format" className="flex h-9 w-full border bg-background px-3 text-sm" value={format} onChange={event => setFormat(event.target.value as TilesetImportFormat)}><option value="grid">Grid (48×48)</option><option value="a2">A2 autotile</option></select></div>
    </div>
...
export function AssetManagerDialog({ open, onOpenChange, game, assetUrls, library, onImportLibrary, onImportLocal, onUpdate, onChangeTerrainCollision }: {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  game: SourceGame;
  assetUrls: Record<string, string>;
  library: LibraryTileset[];
  onImportLibrary: (asset: LibraryTileset) => void;
  ...
  return <>
    <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="h-[92vh] w-[94vw] max-w-none grid-rows-[auto_1fr_auto] gap-0 overflow-hidden p-0 sm:max-w-none">
      <DialogHeader className="border-b px-5 py-4"><DialogTitle className="text-base">Asset Manager</DialogTitle><DialogDescription>Manage project tilesets, inspect source pixels, and edit their default collisions.</DialogDescription></DialogHeader>
      <div className="grid min-h-0 grid-cols-[280px_minmax(0,1fr)]">
        <aside className="flex min-h-0 flex-col border-r bg-muted/10">
          ...
          <div className="grid gap-2 border-t p-3"><Button variant="outline" size="sm" onClick={() => setLibraryOpen(true)}><Library />Open library</Button><Button size="sm" onClick={() => setImportOpen(true)}><Upload />Import PNG</Button></div>
        </aside>
        ...
        {selected ? <section className="flex min-h-0 flex-col">
          ...
          <div className="flex shrink-0 items-center gap-1 border-b bg-background/80 px-5 py-2" role="toolbar" aria-label="Tileset obstacle tools">
            ...
            <span className="ml-3 text-[10px] text-muted-foreground">{selected.kind === 'a2' ? 'Cell blocks the surface; Edge blocks only the exterior boundary of each auto-tiled area.' : 'Click a terrain region or its nearest edge. Red cells and orange edges are blocked.'}</span>
          </div>
        ...
        </section> : <section className="grid place-items-center bg-muted/20"><div className="max-w-xs text-center"><ImagePlus className="mx-auto mb-3 size-7 text-muted-foreground" /><h2 className="text-sm font-medium">No project tilesets</h2><p className="mt-1 text-xs text-muted-foreground">Import a PNG or choose one from the library to get started.</p><div className="mt-4 flex justify-center gap-2"><Button variant="outline" size="sm" onClick={() => setLibraryOpen(true)}>Open library</Button><Button size="sm" onClick={() => setImportOpen(true)}>Import PNG</Button></div></div></section>}
      </div>
      <DialogFooter className="border-t px-5 py-3"><Button variant="outline" onClick={() => onOpenChange(false)}>Close</Button></DialogFooter>
    </DialogContent></Dialog>

    <LibraryDialog open={libraryOpen} onOpenChange={setLibraryOpen} game={game} library={library} onImport={onImportLibrary} />
