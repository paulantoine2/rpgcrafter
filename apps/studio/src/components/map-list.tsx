    ...
    return <div key={map.id} role="treeitem" aria-expanded={children.length ? !collapsed : undefined}>
      ...
      >
        ...
        {editingMapId === map.id
          ...
          /></div>
          : <button type="button" className="flex min-w-0 flex-1 items-center gap-2 px-1 py-0.5 text-left" onClick={() => onSelect(map.id)} onDoubleClick={() => setEditingMapId(map.id)}><MapIcon className="size-3.5 shrink-0 text-primary" /><span className="min-w-0 flex-1 truncate font-medium">{map.name}</span><span className="shrink-0 text-[10px] text-muted-foreground">#{map.numericId}</span></button>}
