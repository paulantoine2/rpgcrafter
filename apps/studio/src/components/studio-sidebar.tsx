      ...
      {planes.map((plane, planeIndex) => {
        ...
        return <div key={plane.id} className="mb-1 last:mb-0">
          <div className={cn('flex h-7 items-center gap-1 px-2 text-xs hover:bg-sidebar-accent/60', activeLayer?.planeId === plane.id && 'bg-primary/10')}>
            ...
            <Popover.Root><Popover.Trigger render={<Button type="button" variant="ghost" size="icon-xs" aria-label={`Plane settings: ${plane.name}`}><Settings2 /></Button>} /><Popover.Portal><Popover.Positioner side="right" align="start" sideOffset={4} className="z-50"><Popover.Popup className="w-60 bg-popover p-2 text-popover-foreground shadow-md ring-1 ring-foreground/10 outline-none">
              <div className="space-y-2"><label className="grid gap-1 text-[10px] text-muted-foreground">Name<Input key={`${plane.id}:${plane.name}`} className="h-7 text-xs" defaultValue={plane.name} onBlur={event => { const name = event.currentTarget.value.trim(); if (name && name !== plane.name) onRenamePlane(plane.id, name); }} /></label><label className="grid gap-1 text-[10px] text-muted-foreground">Coverage<select className="h-7 border bg-background px-1 text-xs" value={plane.surfaceCoverage} onChange={event => onCoverage(plane.id, event.target.value as SurfaceCoverage)}><option value="bounds">Whole map</option><option value="painted">Painted cells</option></select></label><label className="grid gap-1 text-[10px] text-muted-foreground">Surface layer<select className="h-7 border bg-background px-1 text-xs" value={plane.surfaceLayerId} onChange={event => onSurface(plane.id, event.target.value)}>{layers.map(layer => <option key={layer.id} value={layer.id}>{layer.name}</option>)}</select></label><div className="flex justify-end gap-1 border-t pt-2"><Button type="button" variant="ghost" size="icon-xs" disabled={planeIndex >= planes.length - 1} onClick={() => onMovePlane(plane.id, -1)} aria-label="Move plane down"><ArrowDown /></Button><Button type="button" variant="ghost" size="icon-xs" disabled={planeIndex <= 0} onClick={() => onMovePlane(plane.id, 1)} aria-label="Move plane up"><ArrowUp /></Button><Button type="button" variant="ghost" size="icon-xs" disabled={planes.length === 1} onClick={() => onDeletePlane(plane.id)} aria-label="Delete plane"><Trash2 /></Button></div></div>
            </Popover.Popup></Popover.Positioner></Popover.Portal></Popover.Root>
          </div>
          <div className="ml-4 border-l border-border pl-1">{layers.map(layer => { const layerIndex = layers.findIndex(item => item.id === layer.id); return <div key={layer.id} className={cn('flex h-7 items-center gap-1 px-2 text-xs hover:bg-sidebar-accent', layer.id === activeLayer?.id && 'bg-sidebar-accent text-sidebar-accent-foreground')}><button type="button" className="flex min-w-0 flex-1 items-center gap-2 text-left" onClick={() => onSelect(layer.id)}><Layers3 className="size-3 shrink-0 text-primary" /><span className="min-w-0 flex-1 truncate">{layer.name}</span></button><Popover.Root><Popover.Trigger render={<Button type="button" variant="ghost" size="icon-xs" aria-label={`Layer settings: ${layer.name}`}><Settings2 /></Button>} /><Popover.Portal><Popover.Positioner side="right" align="start" sideOffset={4} className="z-50"><Popover.Popup className="w-60 bg-popover p-2 text-popover-foreground shadow-md ring-1 ring-foreground/10 outline-none"><div className="space-y-2"><label className="grid gap-1 text-[10px] text-muted-foreground">Name<Input key={`${layer.id}:${layer.name}`} className="h-7 text-xs" defaultValue={layer.name} onBlur={event => { const name = event.currentTarget.value.trim(); if (name && name !== layer.name) onRenameLayer(layer.id, name); }} /></label><label className="grid gap-1 text-[10px] text-muted-foreground">Plane<select className="h-7 border bg-background px-1 text-xs" value={layer.planeId} disabled={plane.surfaceLayerId === layer.id} onChange={event => onChangeLayerPlane(layer.id, event.target.value)}>{planes.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="grid gap-1 text-[10px] text-muted-foreground">Render phase<select className="h-7 border bg-background px-1 text-xs" value={layer.renderPhase} onChange={event => onChangeLayerPhase(layer.id, event.target.value as RenderPhase)}><option value="belowActors">Below actors</option><option value="aboveActors">Above actors</option></select></label><div className="flex justify-end gap-1 border-t pt-2"><Button type="button" variant="ghost" size="icon-xs" disabled={layerIndex >= layers.length - 1} onClick={() => onMoveLayer(layer.id, -1)} aria-label="Move layer down"><ArrowDown /></Button><Button type="button" variant="ghost" size="icon-xs" disabled={layerIndex <= 0} onClick={() => onMoveLayer(layer.id, 1)} aria-label="Move layer up"><ArrowUp /></Button><Button type="button" variant="ghost" size="icon-xs" disabled={plane.surfaceLayerId === layer.id} onClick={() => onDeleteLayer(layer.id)} aria-label="Delete layer"><Trash2 /></Button></div></div></Popover.Popup></Popover.Positioner></Popover.Portal></Popover.Root></div>; })}</div>
...
function PlanPanel({ map, activePlaneId, onSelect, onAdd, onRename, onMove, onDelete, onCoverage, onSurface }: { map: GameMap; activePlaneId: string; onSelect: (layerId: string) => void; onAdd: () => void; onRename: (id: string, name: string) => void; onMove: (id: string, direction: -1 | 1) => void; onDelete: (id: string) => void; onCoverage: (id: string, coverage: SurfaceCoverage) => void; onSurface: (id: string, layerId: string) => void }) {
  ...
  return <ResizableSidebarSection defaultSize="35%" title="Planes" icon={<Navigation className="size-3.5" />} actions={<Button type="button" variant="ghost" size="icon-xs" onClick={onAdd} aria-label="Add plane"><Plus/></Button>}>
    ...
    {active && <div className="space-y-1 border-t p-1.5"><div className="flex gap-1"><Input className="h-7 min-w-0 flex-1 px-2 text-xs" value={active.name} onChange={event => onRename(active.id, event.target.value)} /><Button variant="ghost" size="icon-xs" disabled={index <= 0} onClick={() => onMove(active.id, -1)}><ArrowDown/></Button><Button variant="ghost" size="icon-xs" disabled={index >= planes.length - 1} onClick={() => onMove(active.id, 1)}><ArrowUp/></Button><Button variant="ghost" size="icon-xs" disabled={planes.length === 1} onClick={() => onDelete(active.id)}><Trash2/></Button></div><div className="grid grid-cols-2 gap-1"><select className="h-7 border bg-background px-1 text-[10px]" value={active.surfaceCoverage} onChange={event => onCoverage(active.id, event.target.value as SurfaceCoverage)}><option value="bounds">Whole map</option><option value="painted">Painted cells</option></select><select className="h-7 border bg-background px-1 text-[10px]" value={active.surfaceLayerId} onChange={event => onSurface(active.id, event.target.value)}>{map.tileLayers.filter(layer => layer.planeId === active.id).map(layer => <option key={layer.id} value={layer.id}>{layer.name}</option>)}</select></div></div>}
  </ResizableSidebarSection>;
}
