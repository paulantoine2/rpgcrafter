...
export function TerrainPreview({ tileset, terrainId, imageUrl }: { tileset: TilesetDefinition; terrainId: string; imageUrl: string }) {
  ...
  if (tileset.kind === 'grid') return <span className="block size-12 bg-no-repeat [image-rendering:pixelated]" data-preview-kind="grid" style={{ backgroundImage: `url(${imageUrl})`, backgroundPosition: `${-terrain.origin.column * 48}px ${-terrain.origin.row * 48}px`, backgroundSize: `${tileset.columns * 48}px ${tileset.rows * 48}px` }} />;
  const previewMask = 0;
  const variant = autotileVariant(tileset, previewMask);
  return <span className="grid size-12 grid-cols-2 overflow-hidden bg-muted [image-rendering:pixelated]" data-preview-kind="a2" data-preview-mask={previewMask}>
