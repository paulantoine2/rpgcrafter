...
describe('conditionalCommands', () => {
  ...
  it('selects else or no commands when the condition fails', () => {
    const command = {
      ...
      condition: { kind: 'variable' as const, id: 1, operator: 'greaterThan' as const, operand: { kind: 'constant' as const, value: 10 } },
  ...
  it('returns nested branches for the command process to execute in order', () => {
    const nested = { type: 'conditional' as const, condition: { kind: 'item' as const, id: 1, amount: 2 }, thenCommands: [{ type: 'save' as const }] };
    const selected = conditionalCommands({ type: 'conditional', condition: { kind: 'switch', id: 1, operand: { kind: 'constant', value: true } }, thenCommands: [{ type: 'toast', text: 'First' }, nested] }, state);
