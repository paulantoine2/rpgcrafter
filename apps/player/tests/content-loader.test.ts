...
describe('Studio preview content', () => {
  it('validates and normalizes a project received from the Studio', () => {
    ...
    preview.maps.village.tileLayers = [{ id: 'ground', name: 'Ground', tileset: 'outside-a2', planeId: 'aubeval', renderPhase: 'belowActors', tiles: [{ x: 2, y: 3, terrainId: 'dirt-on-grass' }] }];
    const loaded = loadStudioPreviewSource(preview);
    expect(loaded.maps.village.tileLayers[0].tiles[0]).toEqual({ x: 96, y: 144, tilesetId: 'outside-a2', terrainId: 'dirt-on-grass' });
  });

  it('rejects an invalid project received from the Studio', () => {
    const preview = source();
    preview.maps.village.planes[0].surfaceLayerId = 'ground';
    preview.maps.village.tileLayers = [{ id: 'ground', name: 'Ground', tileset: 'outside-a2', planeId: 'aubeval', renderPhase: 'belowActors', tiles: [{ x: 2, y: 3, terrainId: 'unknown' }] }];
