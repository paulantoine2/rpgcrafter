...
describe('resolveEventPage', () => {
  it('gives the highest-numbered matching page priority', () => {
    const event: MapEvent = {
      ...
      pages: [page(), page([{ kind: 'switch', id: 1, operand: { kind: 'constant', value: true } }]), page([{ kind: 'switch', id: 2, operand: { kind: 'constant', value: true } }])],
    };
  ...
  it('falls back toward page 1 and returns nothing when no page matches', () => {
    ...
    const firstSwitch = resolveEventPage(event, conditions => conditions.every(condition => condition.kind === 'switch' && condition.id === 1));
